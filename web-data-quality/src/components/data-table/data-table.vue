                        ...
                        <tr v-for="(v,i) in data.conteudo" :key="i">
                            ...
                            <td v-if="config.showOptions">
                               <template v-if="config.showOptionsTYPE == 'FILE'">     
                                     <span v-if="($can('manage', 'FILE-ORIGINAL') && v.original && v.original == 1)" ><i @click="buttonClick('downloadOriginal',v)" style="cursor:pointer;margin-right: 14px;" class="fas fa-file-download" title="Arquivo original"></i></span>                                                               
